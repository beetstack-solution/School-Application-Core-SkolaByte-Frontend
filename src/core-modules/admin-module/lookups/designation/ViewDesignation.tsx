...
import { getDesignationById } from "@/api/admin-api/lookups-api/designationApi";
...
function ViewDesignation() {
  ...
  const fetchDesignationById = async (paramId: string) => {
    try {
      const responseData: any = await getDesignationById(paramId);
      if (responseData.success) {
        setDesignation(responseData.data);
      } else {
        setError("Designation not found.");
      }
    } catch (error) {
      console.error("Error fetching designation:", error);
      setError("Error fetching designation");
    } finally {
      setLoading(false);
    }
  };
  ...
  const breadcrumbItems = [
    { label: "Home", path: "/" },
    { label: "Designations", path: "/lookups/designation" },
    { label: "View Designation", path: "" },
  ];
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">View Designation</h2>
          <div className="mt-2">
            <Breadcrumb items={breadcrumbItems} />
          </div>
        </div>
        <Link
          to="/lookups/designation"
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <tbody className="bg-white divide-y divide-gray-200">
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{designation.code || "N/A"}</td>
                </tr>
                <tr className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Name</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{designation.name}</td>
                </tr>
                <tr className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">Status</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${designation.status
                ...
                <tr className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {designation.createdBy?.name || "N/A"}
                  </td>
                </tr>
              </tbody>
            </table>
