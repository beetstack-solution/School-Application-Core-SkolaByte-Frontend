import { createDesignation } from '@/api/admin-api/lookups-api/designationApi';
...
const AddDepartment: React.FC<AddFeeInstallmentTypeProps> = ({ onClose, onReload }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response: any = await createDesignation(feeTypeData);
            if (response.success) {
            ...
            console.error(error.message || 'Error creating Department:', error);
            setError(error.message ||'Failed to create PTA Role. Please try again.');
        } finally {
    ...
    return (
        <div className="bg-white rounded-xl  p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-semibold text-gray-800">Add Designation</h2>
                <button
                    onClick={onClose}
