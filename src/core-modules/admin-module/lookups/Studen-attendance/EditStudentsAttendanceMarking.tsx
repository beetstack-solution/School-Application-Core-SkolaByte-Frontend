import { AttendanceDTO, updateAttendance, fetchAttendance, fetchAttendanceById } from '@/api/admin-api/student-management/students-api/studentsAttendanceApi'
import { AttendanceStatusType, AttendanceType, Division, fetchAcademicYear, fetchAttendanceStatusDd, fetchAttendanceTypeDd, fetchClasses, fetchDivisionsDD, getStudentsByClassDivisionAcademicYear, Student } from '@/api/common-api/commonDropDownApi'
...
const EditStudentsAttendanceMarking: React.FC = () => {
    ...
    const [studentStatusMap, setStudentStatusMap] = useState<any>({});
    const [updateStudentListId, setUpdateStudentListId] = useState<string | null>(null);
    ...
    console.log("studentStatusMap", studentStatusMap);
    ...
    const studentIdsArray = Object.keys(studentStatusMap);
    const studentIdsString = studentIdsArray.join(", ");

    console.log("Student IDs in studentStatusMap:", studentIdsString);
    console.log("Array of Student IDs:", studentIdsArray);
    ...
    useEffect(() => {
        const fetchAttendanceDataById = async () => {
            ...
            try {
                ...
                data.students.forEach((student: { firstHalfStatus: any; student: { _id: string | number }; remarks: string }) => {
                    if (student.firstHalfStatus) {
                        initialStatusMap[student.student._id] = student.firstHalfStatus;
                    }
                    if (student.remarks) {
                        initialRemarksMap[student.student._id] = student.remarks;
                    }
                });
                data.students.forEach((student: { secondHalfStatus: any; student: { _id: string | number }; remarks: string }) => {
                    if (student.secondHalfStatus) {
                        initialStatusMap[student.student._id] = student.secondHalfStatus;
                    }
                    if (student.remarks) {
                        initialRemarksMap[student.student._id] = student.remarks;
                    }
                ...
                setStudentStatusMap(initialStatusMap);
    ...
    const handleSingleSelect = (studentId: string | number, status: AttendanceStatusType) => {
        setStudentStatusMap((prev: any) => ({
            ...prev,
            [studentId]: prev[studentId]?._id === status._id ? null : status,
        }));
    };
    ...
    const handleSubmit = async () => {
                ...
                if (selectedType.nameAlias === ATTENDANCE_TYPE_MAP.HALF_DAY) {
                    return {
                        ...
                        ...(formData.halfDayTimes.firstHalf === 'morning' && {
                            firstHalfStatus: studentStatusMap[student._id]?._id || '',
                            secondHalfStatus: undefined
                        }),
                        ...(formData.halfDayTimes.secondHalf === 'afternoon' && {
                            firstHalfStatus: undefined,
                            secondHalfStatus: studentStatusMap[student._id]?._id || ''
                        })
                    }
                } else {
                    return {
                        ...baseData,
                        fullDayStatus: studentStatusMap[student._id]?._id || '',
                        firstHalfStatus: undefined,
                        secondHalfStatus: undefined,
                    }
                }
        ...
        try {
            ...
            if (response.success) {
                ...
                navigate(`/student-managements/students-attendance`);
    ...
    return (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {attendanceList.map((student, index) => (
                            <tr key={index}>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-500">
                                    <div className="flex flex-wrap gap-2">
                                        {attendanceStatuses.map((status) => {
                                            const isSelected = studentStatusMap[student._id]?._id === status._id;
                                ...
                                <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                                    {studentStatusMap[student._id]?.name.toLowerCase() === 'absent' && (
                                        <input
                                            type="text"
            ...
            <div className="flex justify-end mt-4">
                <button
                    onClick={() => navigate(`/student-managements/students-attendance`)}
                ...
                <button
                    ...
                    className={`px-6 py-2 me-2 rounded ${formData.academicYear &&
                            formData.class &&
                            formData.division &&
                            formData.date &&
...
export default EditStudentsAttendanceMarking;