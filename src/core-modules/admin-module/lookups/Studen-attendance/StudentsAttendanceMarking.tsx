...
import { AttendanceDTO, createAttendance, fetchAttendance } from '@/api/admin-api/student-management/students-api/studentsAttendanceApi'
import { AttendanceStatusType, AttendanceType, Division, fetchAcademicYear, fetchAttendanceStatusDd, fetchAttendanceTypeDd, fetchClasses, fetchDivisionsDD, getStudentsByClassDivisionAcademicYear } from '@/api/common-api/commonDropDownApi'
...
interface StudentStatus {
  ...
  rollNumber?: string; // Add roll number to the status object
}
type StudentStatusMapType = {
  [studentId: string]: StudentStatus | null;
};


const StudentsAttendanceMarking: React.FC = () => {
  ...
  const [studentStatusMap, setStudentStatusMap] = useState<StudentStatusMapType>({});
...
const [attendanceMarked, setAttendanceMarked] = useState<AttendanceMarked>({
  ...
  const handleFilter = async () => {
    ...
    setError(null);
    try {
      const response = await getStudentsByClassDivisionAcademicYear(formData.class, formData.division, formData.academicYear);
  ...
  // const handleHalfDaySelect = (studentId, part, status) => {
  //   setStudentStatusMap((prev) => ({
  //     ...prev,
  //     [studentId]: {
  //       ...prev[studentId],
  //       [part]: prev[studentId]?.[part]?._id === status._id ? null : status,
  //     },
  //   }));
  // };
  ...
  console.log("Payload Students with Remarks:", attendanceList.map(student => ({
    student: student._id,
    rollNumber: student.rollNumber,
    attendanceRemarks: studentRemarks[student._id] || '',
  })));


 const handleSingleSelect = (studentId: string | number, status: AttendanceStatusType) => {
  const student = attendanceList.find(s => s._id === studentId);
  setStudentStatusMap((prev) => ({
    ...prev,
    [studentId]: {
      status: prev[studentId]?.status?._id === status._id ? null : status,
      rollNumber: student?.rollNumber
    },
  }));
};
  ...
  const handleSubmit = async () => {
    if (!formData.academicYear || !formData.class || !formData.division || !formData.date || !formData.attendanceType) {
      setMessage({
 ...
 const selectedType = attendanceTypes.find(t => t._id === formData.attendanceType);
    ...
    const submissionData: AttendanceDTO = {
      ...
      date: formData.date,
      students: attendanceList.map(student => {
        const baseData = {
          student: student._id,
          rollNumber: student.rollNumber,
          attendanceRemarks: remarksMap[String(student._id)] || "",
        ...
        if (selectedType.nameAlias === ATTENDANCE_TYPE_MAP.HALF_DAY) {
          if (formData.halfDayTimes.firstHalf === 'morning') {
            return {
              ...baseData,
              firstHalfStatus: studentStatusMap[student._id]?.status?._id || undefined,
            ...
            return {
              ...
              secondHalfStatus: studentStatusMap[student._id]?.status?._id || undefined
            }
          }
        }
        ...
        return {
          ...baseData,
          fullDayStatus: studentStatusMap[student._id]?.status?._id || undefined
        }
      }),
  ...
  useEffect(() => {
    ...
    const checkAttendance = async () => {
      if (!formData.date || !formData.class || !formData.division) return;
      ...
      try {
        ...
        const existingRecords = existingResponse.data?.data || [];
         ...
         const hasMorningRecord = existingRecords.some((record: { halfDayTimes: { firstHalf: string }; students: any[] }) =>
        record.halfDayTimes?.firstHalf === 'morning' &&
        record.students.some((student: { firstHalfStatus: null }) => student.firstHalfStatus !== null)
      );


      const hasAfternoonRecord = existingRecords.some((record: { halfDayTimes: { secondHalf: string }; students: any[] }) =>
        record.halfDayTimes?.secondHalf === 'afternoon' &&
        record.students.some((student: { secondHalfStatus: null }) => student.secondHalfStatus !== null)
      );
  ...
  }, [formData.date, formData.class, formData.division]);
...
const areAllPresent = () => {
  ...
  return attendanceList.every(student => 
    studentStatusMap[student._id]?.status?._id === presentStatus._id
  );
};
...
const handleMarkAllPresent = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const newStatusMap = {...studentStatusMap};
  const newRemarksMap = {...remarksMap};

  if (e.target.checked) {
    attendanceList.forEach(student => {
      newStatusMap[student._id] = {
        status: presentStatus,
        rollNumber: student.rollNumber 
      };
      delete newRemarksMap[student._id];
    });
  } else {
    attendanceList.forEach(student => {
      delete newStatusMap[student._id];
    });
  }
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
      ...
      <div className="flex flex-col md:flex-row justify-start items-center px-1 mb-4 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Attendance Marking</h2>
          <div className="mt-1">
  ...
  {bothMarked ? (
    ...
    attendanceList.map((student, index) => (
      <tr key={student._id}>
        ...
        <td className="px-4 py-3 text-sm text-gray-500">
          <div className="flex flex-wrap gap-2">
            {attendanceStatuses.map((status) => {
              const isSelected = studentStatusMap[student._id]?.status?._id === status._id;
              const isAbsent = status.name.toLowerCase() === 'absent';


              return (
                <button
                  key={`${student._id}_${status._id}`}
                  type="button"
                  onClick={() => handleSingleSelect(student._id, status)}
        ...
        <td>
          {studentStatusMap[student._id]?.status?.name.toLowerCase() === 'absent' && (
            <input
              type="text"
...
export default StudentsAttendanceMarking;
