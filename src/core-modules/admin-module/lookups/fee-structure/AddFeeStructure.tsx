...
import { createFeeStructure, FeeStructureData, FeeStructureResponse } from "@/api/admin-api/lookups-api/feeStructureApi";
...
function AddFeeStructure() {
  ...
  const getStudentsByClass = async (classId: string, academicYearId: string) => {
    try {
      const response = await fetchStudentsByClass(classId, academicYearId);
      if (response.success) {
        setStudentsOptions(response.data);
        setFeeStructureData((prev: any) => ({
          ...prev,
          students: response.data.map((student: any) => student._id)
        }));
      } else {
  ...
  useEffect(() => {
    if (feeStructureData.class && feeStructureData.academicYear) {
      getStudentsByClass(feeStructureData.class, feeStructureData.academicYear);
    }
  ...
  const handleFeeAmountChange = (index: number, e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    if (name === "feeType" && !isFeeTypeUnique(value, index)) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const totalAmountFromSplitup = feeStructureData.feeAmountSplitup.reduce((sum: number, splitup: any) => {
      return sum + (parseFloat(splitup.amount) || 0);
    }, 0);

    if (totalAmountFromSplitup !== parseFloat(feeStructureData.totalFee)) {
      toast.error("The total fee does not match the sum of fee amounts.");
      return;
    }
    ...
    try {
      ...
      const response: FeeStructureResponse = await createFeeStructure(payload);
      if (response.success) {
      ...
      toast.error(error.message || 'An error occurred while submitting the fee structure');
    }
  };
  ...
  return (
    <div className=" mx-auto px-4 py-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Add Fee Structure</h3>
