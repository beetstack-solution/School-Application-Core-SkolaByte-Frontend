...
import { fetchSyllabus, SyllabusEntry, updateStatusById, deleteData } from "@/api/admin-api/lookups-api/syllabusApi";
...
function Syllabus() {
  ...
  const loadSyllabus = async () => {
    setIsLoading(true);
    try {
      const { syllabusList: data, total } = await fetchSyllabus(
  ...
  const handleSearch = (query: string) => {
    if (query) {
     ...
     loadSyllabus(); // Reload full list if search query is empty
    }
    setCurrentPage(1);
  };
  ...
  const exportToExcel = async () => {
    const response = await fetchSyllabus(0, Number.MAX_SAFE_INTEGER);
    const data = response.syllabusList.map((syllabus) => {
      // Process subjects and topics
      const subjectsWithTopics = syllabus.subjects.map((subject: any) => {
      ...
      // Combine all subjects and topics for this syllabus
      const allSubjects = subjectsWithTopics.map((s: any) => s.subject).join("; ");
      const allTopics = subjectsWithTopics.map((s: any) => s.topics).join("; ");

      return {
        "Sl No.": response.syllabusList.indexOf(syllabus) + 1,
        "Academic Year": syllabus.academicYear?.academicYear || 'N/A',
        "Class": syllabus.class?.name || 'N/A',
        "Subjects": allSubjects,
        "Topics": allTopics,
        "Status": syllabus.status ? "Active" : "Inactive",
        "CreatedAt": formatDate(syllabus.createdAt),
      };
    });
    ...
    XLSX.writeFile(wb, `syllabus_${new Date().toISOString()}.xlsx`);
  }
    ...
    try {
      ...
      if (responseData?.success) {
        toast.success(
          ...
          `syllabus status updated to ${newStatus ? "Active" : "Inactive"}.`
        );
        loadSyllabus();
      } else {
        console.error(
          "Updated syllabus is undefined or missing _id",
          responseData
        );
        toast.error(
          responseData.message ||
          "Failed to update module status due to missing syllabus data."
        );
      }
    } catch (error: any) {
  ...
  const handleDelete = async (moduleId: string) => {
    ...
    if (!confirmDelete) {
      return; // Exit the function if the syllabus cancels
    }
    ...
    try {
      ...
      if (response.success === true) {
        toast.success(response.message || "syllabus deleted successfully!");
        loadSyllabus();
      } else {
        toast.error(
          response.message || "Failed to delete syllabus. Please try again."
        );
      }
    } catch (error: any) {
      console.error("Error deleting syllabus:", error);
      toast.error(
        "An error occurred while deleting the syllabus. Please try again."
      );
    }
  };
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        ...
        />
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Syllabus List</h2>
          <div className="mt-1">
        ...
        <div className="flex flex-col md:flex-row items-end md:items-center w-full md:w-auto gap-3">
          <Link to={'/lookups/syllabuses/add'} className="w-full md:w-auto">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                syllabusList.map((syllabus, index) => (
                  <tr key={syllabus._id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-500">
                      {(currentPage - 1) * itemsPerPage + index + 1}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link to={`/lookups/syllabuses/view/${syllabus._id}`}>
                          <GrOverview size={20} title="View" className="text-gray-500 hover:text-blue-600" />
                        </Link>
                        <Link to={`/lookups/syllabuses/edit/${syllabus._id}`}>
                    ...
                    <td className="px-4 py-3">
                      <button
                        className={`px-2 py-0.5 rounded-full text-xs font-medium ${syllabus.status
                          ? "bg-green-100 text-green-800"
                          : "bg-red-100 text-red-800"
                          }`}
                        onClick={() => handleStatusUpdate(syllabus._id, syllabus.status)}
                      >
                        {syllabus.status ? "Active" : "Inactive"}
                      </button>
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500">
                      {syllabus.academicYear?.academicYear || 'N/A'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">{syllabus.code}</td>

                    {/* <td className="px-4 py-3 text-sm text-gray-500">
                      {syllabus.academicYear?.startMonth || 'N/A'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500">
                      {syllabus.academicYear?.endMonth || 'N/A'}
                    </td> */}
                    <td className="px-4 py-3 text-sm text-gray-500">
                      {syllabus.class?.name || 'N/A'}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-500">
