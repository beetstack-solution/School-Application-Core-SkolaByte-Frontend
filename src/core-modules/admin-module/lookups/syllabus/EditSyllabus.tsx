...
import { updateSyllabusById, SyllabusListResponse, fetchSyllabousById } from '@/api/admin-api/lookups-api/syllabusApi'
...
interface SyllabusData {
    academicYear: string | { id: string }; // Can be either string or object with id
...
const EditSyllabus: React.FC = () => {
    ...
    const [initialData, setInitialData] = useState<SyllabusData | null>(null);

    const breadcrumbItems = [
        { label: "Home", path: "/" },
        { label: "Syllabus", path: "/lookups/syllabuses" },
        { label: "Edit Syllabus", path: "" },
    ];

    useEffect(() => {
        const fetchData = async () => {
            try {
                const [yearsData, classesData, subjectsData, syllabusData] = await Promise.all([
                    fetchAcademicYear(),
                    fetchClasses(),
                    fetchSubjects(),
                    fetchSyllabousById(id!),
                ]);
                ...
                if (syllabusData.success) {
                    setFormData(syllabusData.data);
                    setInitialData(syllabusData.data);
                } else {
                    toast.error(syllabusData.message || 'Failed to fetch syllabus data.');
                }
            } catch (error) {
    ...
    const handleSubjectChange = (subjectIndex: number, subjectId: string) => {
        // Check if the subject is already selected in the current form data
        ...
        // Update the subject if it's not a duplicate
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        // Compare with initial data to avoid unnecessary updates
        ...
        try {
            const response: SyllabusListResponse = await updateSyllabusById(id!, formData as any);

            if (response.success) {
                toast.success(response.message || 'Syllabus updated successfully');
                navigate('/lookups/syllabuses');
            } else {
                toast.error(response.message || 'Failed to update syllabus');
            }
        } catch (error) {
            console.error('Error updating syllabus:', error);
            toast.error('An error occurred while updating syllabus');
        }
    };
    return (
        <div className="px-4">
       ...
       <div>
           <h3 className="text-xl font-semibold mb-4">Edit Syllabus</h3>
            ...
            <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow">
                <div className="flex flex-wrap -mx-2 mb-6">
                    <div className="w-full md:w-1/3 px-2 mb-4">
                        ...
                        {/* <select
                            name="academicYear"
                            value={typeof formData.academicYear === 'object' ? formData.academicYear.id : formData.academicYear}
                    ...
                    <div className="w-full md:w-1/3 px-2 mb-4">
                        ...
                        <select
                            name="class"
                            value={typeof formData.class === 'object' ? formData.class.id : formData.class}
                            onChange={handleInputChange}
                            className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                            required
                        >
                                    ...
                                    <React.Fragment key={subjectIndex}>
                                        <tr className="hover:bg-gray-50">
                                            <td className="px-4 py-2 border border-gray-300" rowSpan={subject.topics.length + 1}>
                                                ...
                                                <select
                                                        value={typeof subject.subject === 'object' ? subject.subject.id : subject.subject} // subject.subject is a string
                                                        onChange={(e) => handleSubjectChange(subjectIndex, e.target.value)} // Pass the string value
                                                        className="w-full p-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                                        required
                                                    >
                                                        ...
                                                        {subjects.map(sub => (
                                                            <option
                                                                key={sub._id}
                                                                value={sub._id} 
                                                                // disabled={formData.subjects.some(s => s.subject === sub._id)} // Check for duplicates
                                                            >
                                                                {sub.name}
                                                            </option>
                                                        ))}
