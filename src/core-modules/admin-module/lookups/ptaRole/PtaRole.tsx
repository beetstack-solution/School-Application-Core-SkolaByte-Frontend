    ...
    const handleSearch = async (query: string) => {
        ...
        try {
            if (query.trim()) {
                ...
                const response: any = await getPtaRoles(currentPage, itemsPerPage);
                if (response.success) {
                    // Then filter the data based on search query
                    const filtered = response.data.data.filter((item: any) => {
                        const searchTerm = query.toLowerCase();
                        return (
                            item.name?.toLowerCase().includes(searchTerm) ||
                            (item.code?.toLowerCase().includes(searchTerm)) ||
                            (item.createdBy?.name?.toLowerCase().includes(searchTerm)) ||
                            (item.createdBy?.email?.toLowerCase().includes(searchTerm))
                        );
                    });
