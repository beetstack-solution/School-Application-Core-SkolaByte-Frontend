import { deleteTransportVehicle, fetchTransportVehicles, updateTransportVehicleStatus } from '@/api/admin-api/lookups-api/vehicleTypeApi';
...
function VehicleType() {
    ...
    const getAllTransportVehicles = async (page: number, limit: number) => {
        ...
        try {
            const response: any = await fetchTransportVehicles(page, limit);
            if (response.success) {
    ...
    const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
        ...
        try {
            const response: any = await updateTransportVehicleStatus(_id, newStatus);
            if (response?.success) {
                toast.success(
                    response.message || `Transport Vehicles status updated to ${newStatus ? "Active" : "Inactive"}.`
                );
                getAllTransportVehicles(currentPage, itemsPerPage);
            } else {
                toast.error(response.message || "Failed to update Transport Vehicles status.");
            }
        } catch (error: any) {
            toast.error(error.message || "Failed to update Transport Vehicles status.");
        }
    };
    ...
    const handleSearch = (query: string) => {
        setLoading(true);
        if( query.trim()) {
           const filteredData = filteredTransportVehicles.filter((item: any) =>
    ...
    const handleDelete = async (feeStructureId: string) => {
        ...
        try {
            const response: any = await deleteTransportVehicle(feeStructureId);
            if (response.success) {
       ...
       />
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                    ...
                    <div>
                        <h2 className="text-2xl font-bold text-gray-800">Transportation Type</h2>
                        <div className="mt-1">
                    ...
                    <div className="flex flex-col md:flex-row items-end md:items-center w-full md:w-auto gap-3">
                        <Link to="" onClick={onReturn} className="w-full md:w-auto">
                                    ...
                                    <tr className="hover:bg-gray-50" key={item._id} >
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <Link to={`/lookups/vehicle-types/view/${item._id}`}>
