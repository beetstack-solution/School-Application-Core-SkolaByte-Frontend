...
import { fetchAcademicYearById, updateAcademicYearById } from '@/api/admin-api/lookups-api/academicYearApi';
...
const EditAcademicYear: React.FC<EditAcademicYearProps> = ({ academicYearId,onClose, onReload }) => {
  ...
  useEffect(() => {
    const fetchAcademicYearData = async () => {
      try {
        const response:any = await fetchAcademicYearById(academicYearId);
        if (response.success) {
          setAcademicYear(response.data?.academicYear);
          setStartMonth(response.data?.startMonth);
          setEndMonth(response.data?.endMonth);
        } else {
          toast.error(response.message || 'Failed to fetch academic year data.');
        }
      } catch (error) {
        console.error('Error fetching academic year data:', error);
        toast.error('Error fetching academic year data. Please try again.');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await updateAcademicYearById(academicYearId, updatedAcademicYearData as any);
      if (response.success) {
        ...
        setError(response.message || 'Failed to update Academic Year.');
      }
    } catch (error) {
      console.error('Error updating Academic Year:', error);
      setError('Failed to update Academic Year. Please try again.');
    }
  };
  return (
    <div>
      ...
      {/* {error && <div className="mb-4 text-red-500"></div>} */}
