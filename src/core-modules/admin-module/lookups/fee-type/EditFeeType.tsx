...
import { updateFeeTypeById, getFeeTypeById } from '@/api/admin-api/lookups-api/feeTypeApi';
...
const EditFeeType: React.FC<EditFeeTypeProps> = ({ onClose, onReload, feeTypeId }) => {
    ...
    const fetchFeeTypeById = async (id: string) => {
        try {
            const response: any = await getFeeTypeById(id);
            if (response.success) {
                setType(response.data.name);
            } else {
                console.error('Error in API response:', response.message);
                toast.error(response.message || 'Failed to fetch fee installment type.');
                setError(response.message || 'Failed to fetch fee installment type.');
            }
        } catch (error: any) {
            console.error('Error fetching fee installment type:', error);
            setError('Failed to fetch fee installment type. Please try again.');
        }
    };
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response: any = await updateFeeTypeById(feeTypeId, feeTypeData);
            if (response.success) {
