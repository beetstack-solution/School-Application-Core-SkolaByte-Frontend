...
import { createFeeType } from '@/api/admin-api/lookups-api/feeTypeApi';
...
const AddFeeType: React.FC<AddFeeInstallmentTypeProps> = ({ onClose, onReload }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response:any = await createFeeType(feeTypeData);
      if (response.success) {
  ...
  return (
    <div>
      <div className="flex justify-between">
        <h2 className="text-xl font-bold mb-4">Add Fee Type</h2>
