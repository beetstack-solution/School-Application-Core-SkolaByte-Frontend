import { fetchVehicleById, updateVehicle } from '@/api/admin-api/lookups-api/vehicleInfoApi';
...
function EditVehicleInfo() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                // First load the dropdown options
                const vehicleTypesResponse = await fetchVehicleTypeDD();
                if (!isMounted) return;

                if (vehicleTypesResponse.success) {
                    setVehicleTypeOptions(vehicleTypesResponse.data);

                    // Then load the vehicle data if ID exists
                    if (id) {
                        const vehicleResponse: any = await fetchVehicleById(id);
                        if (!isMounted) return;

                        if (vehicleResponse.data) {
                            setVehicleData(vehicleResponse.data);
                            const vehicleInfo = vehicleResponse.data.vehicleInfo[0];

                            setFormData({
                                academicYear: vehicleResponse.data.academicYear.id,
                                vehicleInfo: {
                                ...
                                driver: {
                                    ...
                                    mobileNumber: vehicleInfo.driver.mobileNumber,
                                    address: vehicleInfo.driver.address,
                                    licenseNumber: vehicleInfo.driver.licenseNumber,
                                },
                                assistant: {
                                    name: vehicleInfo.assistant.name,
                                    mobileNumber: vehicleInfo.assistant.mobileNumber,
                                },
                                route: {
                                    routeName: vehicleInfo.route.routeName,
                                    startTiming: vehicleInfo.route.startTiming,
                                    exactEndTiming: vehicleInfo.route.exactEndTiming,
                                },
                            });
                        } else {
                            toast.error(vehicleResponse.message || 'Failed to fetch vehicle info');
                        }
                    }
                } else {
                    toast.error(vehicleTypesResponse.message || 'Failed to fetch vehicle types');
                }
            } catch (error: any) {
                if (isMounted) {
        ...
        return () => {
            isMounted = false; // Cleanup function to set flag false when unmounting
        };
    }, [id]);
    ...
    const handleChange = (section: keyof FormData, field: string, value: string) => {
        setFormData((prev:any) => ({
            ...
            [section]: {
                ...prev[section],
                [field]: value  // ✅ Now using the actual input value
            }
        }));
    };
    ...
    return (
        <div className="container mx-auto px-4 py-6">
            <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                <div>
                    <h3 className="text-2xl font-bold text-gray-800 mb-2">Edit Vehicle Info</h3>
