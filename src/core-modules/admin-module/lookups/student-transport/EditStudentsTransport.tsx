...
import { updateStudentTransportInfo, getStudentTransportInfoById } from '@/api/admin-api/lookups-api/studenttransportInfoApi';
import { FaPlus, FaMinus } from 'react-icons/fa';

function EditStudentsTransport() {
    const { id } = useParams();
    ...
    const fetchStudentTransportInfoById = async (id: string) => {
        try {
            const response = await getStudentTransportInfoById(id);
            const studentTransportInfo: any = response.data;
            
            setFormData({
                class: studentTransportInfo.class._id,
                division: studentTransportInfo.division._id,
                academicYear: studentTransportInfo.studentTransportMapping[0].academicYear._id,
                studentTransportMappings: studentTransportInfo.studentTransportMapping.map((mapping: any) => ({
                    studentId: mapping.studentId,
                    place: mapping.place,
                    dropLocation: mapping.dropLocation,
                    vehicleId: mapping.vehicleId._id,
                    vehicleInfoId: mapping.vehicleInfo._id,
                    _id: mapping._id // Keep the mapping ID for updates
                }))
            });

            // Fetch drivers for each vehicle in the mappings
            const drivers = await Promise.all(
                studentTransportInfo.studentTransportMapping.map((mapping: any) => 
                    fetchVehicledriveDD(mapping.vehicleId._id)
                )
            );
            setVehicleDriveOptions(drivers.map(response => response.data));
    ...
    useEffect(() => {
        if (id) {
            fetchStudentTransportInfoById(id);
        }
    }, [id]);
    ...
    const addMapping = () => {
        ...
        // Add empty drivers array for the new mapping
        setVehicleDriveOptions((prev: any) => [...prev, []]);
    };
    ...
    const removeMapping = (index: number) => {
        ...
        // Remove the corresponding drivers array
        setVehicleDriveOptions((prev: any) => {
            const newDrivers = [...prev];
        ...
        try {
            ...
            const submissionData: any = {
                ...
                studentTransportMapping: formData.studentTransportMappings.map((mapping: any) => ({
                    _id: mapping._id, // Include the mapping ID for updates
            ...
            const response = await updateStudentTransportInfo(id ?? '', submissionData);
            if (response.success) {
                setSubmitSuccess(true);
                toast.success(response.message || 'Transport information updated successfully!');
                navigate('/lookups/students-transports/');
            } else {
                toast.error(response.message || 'Failed to update transport information');
            }
        } catch (error: any) {
    ...
    const breadcrumbItems = [
        ...
        { label: "Edit Student Transportation", path: "" },
    ];
        ...
        <div className="mx-auto p-6 bg-gray-50 min-h-screen">
            <div className="mx-auto">
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h3 className="text-xl font-semibold mb-4">Edit Student Transportation</h3>
...
export default EditStudentsTransport;