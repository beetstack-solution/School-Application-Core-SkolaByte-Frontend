...
import { deleteTimeSlot, getTimeSlots, updateTimeSlotStatus } from '@/api/admin-api/lookups-api/timeSlotApi';

function TimeSlot() {
    const [feeStructures, setFeeStructures] = useState<any[]>([]);
    const [filteredFeeStructures, setFilteredFeeStructures] = useState<any[]>([]);
    ...
    const fetchAllFeeType = async (page: number, limit: number) => {
        setLoading(true);
    ...
    useEffect(() => {
        fetchAllFeeType(currentPage, itemsPerPage);
    ...
    const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
        ...
        try {
            const response:any = await updateTimeSlotStatus(_id, newStatus);
            if (response?.success) {
                toast.success(
    ...
    const handleDelete = async (feeStructureId: string) => {
        ...
        try {
            const response:any = await deleteTimeSlot(feeStructureId);
            if (response.success) {
    ...
    return (
       ...
       />
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-2">
            ...
            <div>
  <h2 className="text-2xl font-bold text-gray-800">Time Slot</h2>
    <div className="mt-1">
                        ...
                        {filteredFeeStructures?.map((item, index) => (
                            <tr className="hover:bg-gray-50" key={item._id} >
                                ...
                                <div className="flex items-center gap-2">
                                    <Link to={`/lookups/time-slot/view/${item._id}`}>
...
className="text-gray-500 hover:text-blue-600 transition-colors"
                        ...
                        : "bg-red-100 text-red-800"
                                ...
                                <td className="px-4 py-3 text-sm text-gray-500 truncate max-w-xs">{item.startTime}</td>
                                <td className="px-4 py-3 text-sm text-gray-500 truncate max-w-xs">{item.endTime}</td>
            ...
            {showModal && (
                <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
                    <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                        <AddTimeSlot onClose={handleClose} onReload={handleReload} />
                    </div>
                </div>
            )}
