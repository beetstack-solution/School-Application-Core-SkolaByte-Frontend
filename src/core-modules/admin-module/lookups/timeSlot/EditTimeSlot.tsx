...
import { getTimeSlotById, updateTimeSlot } from '@/api/admin-api/lookups-api/timeSlotApi';

interface EditTimeSlotProps {
    onClose: () => void;
    onReload: () => void;
    timeSlotId?: string; // Changed from feeTypeId to timeSlotId for clarity
}

const EditTimeSlot: React.FC<EditTimeSlotProps> = ({ onClose, onReload, timeSlotId }) => {
    ...
    const fetchTimeSlotById = async (id: string) => {
        try {
            ...
            if (response.success) {
                ...
                toast.error(response.message || 'Failed to fetch time slot.');
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (!timeSlotId) {
                throw new Error('Time slot ID is missing');
            }

            const response: any = await updateTimeSlot(timeSlotId, timeSlotData);
            if (response.success) {
                ...
                setError(response.message || 'Failed to update time slot.');
            }
        } catch (error: any) {
            console.error('Error updating time slot:', error);
            setError(error.message || 'Failed to update time slot. Please try again.');
        }
    };

    useEffect(() => {
        if (timeSlotId) {
            fetchTimeSlotById(timeSlotId);
        }
    }, [timeSlotId]);
