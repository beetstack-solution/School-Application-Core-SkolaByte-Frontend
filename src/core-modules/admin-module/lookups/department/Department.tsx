import { getDepartments, deleteDepartment, updateDepartmentStatus  } from '@/api/admin-api/lookups-api/departmentApi';
...
function Department() {
    ...
    const getAllPtaRoles = async (page: number, limit: number) => {
        ...
        try {
            const response: any = await getDepartments(page, limit);
            if (response.success) {
    ...
    const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
        ...
        try {
            const response: any = await updateDepartmentStatus(_id, newStatus);
            if (response?.success) {
                toast.success(
    ...
    const handleSearch = (query: string) => {
       setLoading(true);
        if (query.trim()) {
            const response = filteredTransportVehicles.filter((item: any) =>
                item.name.toLowerCase().includes(query.toLowerCase()) ||
                item.code.toLowerCase().includes(query.toLowerCase())
            );
            setFilteredTransportVehicles(response);
    ...
    const handleDelete = async (feeStructureId: string) => {
        ...
        try {
            const response: any = await deleteDepartment(feeStructureId);
            if (response.success) {
                                    ...
                                    <tr className="hover:bg-gray-50" key={item._id} >
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-2">
                                                <Link to={`/lookups/department/view/${item._id}`}>
                ...
                {showModal && (
                    <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
                        <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
                            <AddDepartment onClose={handleClose} onReload={handleReload} />
                        </div>
                    </div>
                )}
