...
import { deleteNotificationModuleTemplate, fetchAllNotificationModule, updateModuleStatusById, Module } from "@/api/super-admin-api/notifications/notificationModuleApi";
import AddNotificationModule from "./AddNotificationModule";
import EditNotificationModule from "./EditNotificationModule";

function NotificationModule() {
  const [functionality, setFunctionality] = useState<Module[]>([]);
  const [filteredModule, setFilteredModule] = useState<Module[]>([]); // For search and pagination
  ...
  const getAllFunctionality = async () => {
    try {
      ...
      const notifications = data.modules || []; // Ensure it's an array
  ...
  // Handle sorting
  const changeSort = (column: keyof Module) => {
    let order = "ascending";
  ...
  const handleStatusUpdate = async (_id: string, currentStatus: boolean) => {
    ...
    try {
      const responseData = await updateModuleStatusById(_id, newStatus);
      ...
      if (responseData.success) {
        ...
        console.error(
          "Updated Notification ModuleData is undefined or missing _id",
          responseData
        );
        toast.error(
          responseData.message ||
          "Failed to update module status due to missing Notification ModuleData data."
        );
      }
    } catch (error: any) {
  ...
  const handleDelete = async (dealId: string) => {
    const confirmDelete = window.confirm(
      "Are you sure you want to delete this Notification ModuleData?"
    );

    if (confirmDelete) {
      try {
        const response = await deleteNotificationModuleTemplate(dealId);
        if (response.success) {
          ...
          toast.success(
            response.message || "Notification ModuleData deleted successfully!"
          );
          getAllFunctionality();
        } else {
          toast.error(
            response.message ||
            "Failed to delete Notification ModuleData. Please try again."
          );
        }
      } catch (error: any) {
        console.error("Error deleting Notification ModuleData:", error);
        toast.error(
          "An error occurred while deleting the Notification ModuleData. Please try again."
        );
      }
    }
  };
  ...
  {[
    ...
    >
      <div className="flex items-center gap-1">
        {header.name}
        {header.sortKey && sortState.column === header.sortKey.toLowerCase() && sortState.order === "ascending" && (
            ...
            {currentItems.map((module, index) => (
              <tr className="border" key={module._id}>
                ...
                <td className="px-4 py-3">
                  ...
                  <Link to={`/notifications/notification-module/view/${module._id}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            <AddNotificationModule refreshData={getAllFunctionality} onClose={handleClose} />
          </div>
        </div>
      )}
      {/* Edit Tax Modal */}
      {showEditModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            <EditNotificationModule moduletabledata={getAllFunctionality}
              moduleId={selectedId}
