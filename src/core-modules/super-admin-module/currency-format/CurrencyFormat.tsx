...
function CurrencyFormat() {
  const [subGroup, setSubGroup] = useState<CurrencyFormatData[]>([]);
  ...
  const getCurrencyFormts = async () => {
    try {
      ...
      setSubGroup(data.currencyFormats);
      setFilteredSubGroup(data.currencyFormats); // Initialize filteredSubGroup with all subGroup
    } catch (error: any) {
  ...
  const handleSearch = (query: string) => {
    if (query) {
      const filtered = subGroup.filter(
        (module) =>
          module.formatPattern.toLowerCase().includes(query.toLowerCase()) ||
  ...
  // Handle sorting
  const changeSort = (column: keyof CurrencyFormatData) => {
    let order = "ascending";
  ...
  const handleDelete = async (moduleId: string) => {
    ...
    try {
      ...
      if (response.success === true) {
        ...
        toast.error(
          response.message || "Failed to delete Sub Group. Please try again."
        );
      }
    } catch (error: any) {
        ...
        <table className="min-w-full data_table">
          <thead className="bg-gray-700 text-white">
            <tr>
              ...
              <th
                onClick={() => changeSort("formatPattern")}
                style={{ cursor: "pointer" }}
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  Group Name
                  {sortState.column === "formatPattern" &&
                    sortState.order === "ascending" && <AiFillCaretUp />}
                </div>
              </th>
              <th
            ...
            {currentItems.map((module, index) => (
              <tr className="border" key={module._id}>
                ...
                <td className="action-icons">
                  <Link to={`/currency-format/view/${module._id}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            ...
            <AddcurrencyFormat onClose={handleClose} onReload={handleReload} />
          </div>
        </div>
      )}
      {/* Edit Tax Modal */}
