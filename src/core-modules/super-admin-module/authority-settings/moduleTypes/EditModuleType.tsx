...
import { updateDataById, fetchDataById } from "@/api/super-admin-api/authority-setting-api/ModuleTypeApi";
...
const EditModuleType: React.FC<EditCurrencyFormatProps> = ({ onClose, formatId, onReload }) => {
  ...
  useEffect(() => {
    const loadCurrencyFormat = async () => {
      try {
        const response = await fetchDataById(formatId);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await updateDataById(formatId, updatedData);
      if (response.success) {
        toast.success(response.message || "Module type created successfully!");
      } else {
        toast.error(response.message || "Failed to create Module type. Please try again.");
        setError(response.error || "Failed to create Module type. Please try again.");
      }
      //   console.log("Currency format updated successfully!", response);
  ...
  return (
    <div>
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-bold mb-4">Edit Module Type</h2>
