...
} from "@/api/super-admin-api/authority-setting-api/ModuleActionsApi";
...
import AddModuleActions from "./AddModuleActions";
// import AddPermissionModules from "./AddPermissionModules";

function ModuleActions() {
  const [subGroup, setSubGroup] = useState<moduleActionsData[]>([]);
  const [filteredSubGroup, setFilteredSubGroup] = useState<moduleActionsData[]>(
    []
    ...
    if (query) {
      const filtered = subGroup.filter(
        (module) =>
          ...
          module.moduleRef.toLowerCase().includes(query.toLowerCase())
        //   module.moduleType.toLowerCase().includes(query.toLowerCase()) ||
  ...
  // Handle sorting
  const changeSort = (column: keyof moduleActionsData) => {
    let order = "ascending";
  ...
  //     try {
  //       const response = await deleteCurrencyFormat(moduleId);
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1">
        ...
        <table className="min-w-full data_table">
          <thead className="bg-gray-700 text-white">
            <tr>
              ...
              {/* <th>Actions</th> */}
              <th
                onClick={() => changeSort("moduleRef")}
                style={{ cursor: "pointer" }}
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  Module
                  {sortState.column === "moduleRef" &&
                    sortState.order === "ascending" && <AiFillCaretUp />}
                </div>
              </th>
              <th
              ...
              {/* <th
                onClick={() => changeSort("moduleType")}
                style={{ cursor: "pointer" }}
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                Module Type
                  {sortState.column === "moduleType" &&
                    sortState.order === "ascending" && <AiFillCaretUp />}
                </div>
              </th> */}
              {/* <th
            ...
            {currentItems.map((module, index) => (
              <tr className="border" key={module._id}>
                ...
                {/* <td className="action-icons">
                  <Link to={`/view-currency-format/${module._id}`}>
      ...
      {showModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            ...
            <AddModuleActions onClose={handleClose} onReload={handleReload} />
          </div>
        </div>
      )}
      {/* Edit Tax Modal */}
      {/* {showEditModal && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            <EditCurrencyFormat formatId={selectedId} onClose={handleCloseEdit}  onReload={handleReload}/>{" "}
            Pass correct props
          </div>
        </div>
      )} */}
