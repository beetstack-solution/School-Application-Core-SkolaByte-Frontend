...
} from "@/api/super-admin-api/authority-setting-api/PermissionModuleApi";
...
function PermissionModules() {
  const [subGroup, setSubGroup] = useState<PermissionModuleData[]>([]);
  ...
  // Handle sorting
  const changeSort = (column: keyof PermissionModuleData) => {
    let order = "ascending";
        ...
        <table className="min-w-full data_table">
          <thead className="bg-gray-700 text-white">
            <tr>
              ...
              <th
                onClick={() => changeSort("moduleAlias")}
                style={{ cursor: "pointer" }}
              >
                <div style={{ display: "flex", alignItems: "center" }}>
                  Module Alias
                  {sortState.column === "moduleAlias" &&
                    sortState.order === "ascending" && <AiFillCaretUp />}
                </div>
              </th>
              <th
