...
import {
  ...
  fetchFeatures,
  FeatureData,
} from "@/api/super-admin-api/FeatureApi"; // Adjust import path if necessary
...
const EditFeature: React.FC<EditFeatureModuleProps> = ({
  featureId,
  onClose,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(
          response.message || "Failed to update feature. Please try again."
        );
      }
    } catch (error: any) {
      console.error(error.message || "Error updating feature:", error);
      toast.error(
        error.message || "Failed to update feature. Please try again."
      );
      setError(error.message || "Failed to update feature. Please try again.");
    }
  };
