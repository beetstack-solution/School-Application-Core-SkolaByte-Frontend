...
import {
  ...
  updateFeatureStatus,
  deleteFeature,
} from "@/api/super-admin-api/FeatureApi"; // Adjusted to import FeatureApi
...
import AddFeature from "./AddFeature"; // Import the AddFeature modal
import Pagination from "@/components/Pagination";

function Feature() {
  const [features, setFeatures] = useState<FeatureData[]>([]);
  const [filteredFeatures, setFilteredFeatures] = useState<FeatureData[]>([]);
  ...
  const changeSort = (column: keyof FeatureData) => {
    let order = "ascending";
            ...
            {currentItems.map((item, index) => (
              <tr className="border" key={item._id}>
                ...
                <td className="action-icons">
                  <Link to={`/feature/view/${item._id}`}>
                    <GrOverview size={25} title="View" className="view-icon" />
                  </Link>
                  
                    <CiEdit size={28} title="Edit"
                     className="edit-icon" 
...
{showEdit && (
        <div className="fixed inset-0 flex justify-center items-center bg-gray-900 bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-lg">
            <EditFeature featureId={selectedId} onClose={handleCloseEdit}  onReload={handleReload} />{" "}
          </div>
        </div>
      )}
